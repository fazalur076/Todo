...
if [ ! -d "$DIR/Todo.app" ] || [ ! -f "$DIR/Todo.app/Contents/MacOS/Todo" ]; then
    if [ "$MODE" == "gui" ]; then
...
if [ -w "/Applications" ]; then
    rm -rf "$TARGET"
    cp -R "$DIR/Todo.app" "$TARGET"
else
    osascript -e "do shell script \"rm -rf '$TARGET' && cp -R '$DIR/Todo.app' '$TARGET'\" with administrator privileges"
fi
